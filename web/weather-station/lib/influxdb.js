...
export async function fetchCurrentData() {
  ...
  try {
    ...
    const queryTemp = `
      from(bucket: "${bucket}")
        |> range(start: -8m)
        |> filter(fn: (r) => r._measurement == "sensors" and r._field == "temperatura_exterior_C")
        |> last()
    `
    ...
    const queryHum = `
      from(bucket: "${bucket}")
        |> range(start: -8m)
        |> filter(fn: (r) => r._measurement == "sensors" and r._field == "humedad_exterior_pct")
        |> last()
    `

    const [tempResult, humResult] = await Promise.all([
      queryApi.collectRows(queryTemp),
      queryApi.collectRows(queryHum)
    ])

    return {
      temperatura_exterior_C: tempResult[0]?.["_value"] ?? 0,
      humedad_exterior_pct: humResult[0]?.["_value"] ?? 0,
    }
  } catch (error) {
  ...
  try {
    ...
    const query = `
      from(bucket: "${bucket}")
        ...
        |> filter(fn: (r) => r._field == "temperatura_exterior_C" or r._field == "humedad_exterior_pct")
    ...
    return result.map((row) => ({
      time: row._time,
      temperatura_exterior_C: row.temperatura_exterior_C !== undefined ? Number.parseFloat(row.temperatura_exterior_C || 0).toFixed(1) : null,
      humedad_exterior_pct: row.humedad_exterior_pct !== undefined ? Number.parseFloat(row.humedad_exterior_pct || 0).toFixed(1) : null,
    }))
  } catch (error) {
